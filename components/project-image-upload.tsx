...
export function ProjectImageUpload({ onImageUploaded }: ProjectImageUploadProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ("error" in response) {
        ...
        if (onImageUploaded && response.url) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="project-image">Upload Project Screenshot</Label>
        <Input id="project-image" type="file" onChange={handleFileChange} accept="image/*" disabled={uploading} />
        <p className="text-xs text-muted-foreground">
          Upload screenshots of your website to showcase in your portfolio
        </p>
      </div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium">
              {result.success ? "Screenshot uploaded successfully!" : `Error: ${result.error}`}
            </p>
          </div>
